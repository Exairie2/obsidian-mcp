...
function createBacklinkMatches(
  content: string,
  noteIndex: NoteInfo[],
  options: {
    minLength: number;
  ...
  function isInSkipRegion(start: number, end: number): boolean {
    return skipRegions.some((region) => (start >= region.start && start < region.end) || (end > region.start && end <= region.end) || (start <= region.start && end >= region.end));
  }
  ...
  for (const note of noteIndex) {
    ...
    try {
      ...
      while ((match = regex.exec(processedContent)) !== null) {
        ...
        processedContent = processedContent.substring(0, matchStart) + `[[${note.nameWithoutExt}]]` + processedContent.substring(matchEnd);
...
// 查找标题位置
function findHeadingPosition(elements: MarkdownElement[], heading: string, level?: number): { index: number; element: MarkdownElement } | null {
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    if (element.type === "heading") {
      ...
      // 标准化后匹配
      const normalizedContent = element.content
        .toLowerCase()
        .replace(/[^\w\s]/g, "")
        .replace(/\s+/g, " ");
...
// 查找块ID位置
function findBlockIdPosition(elements: MarkdownElement[], blockId: string): { index: number; element: MarkdownElement } | null {
  for (let i = 0; i < elements.length; i++) {
    const element = elements[i];
...
// 计算插入位置
function calculateInsertPosition(elements: MarkdownElement[], targetIndex: number, position: "before" | "after" | "append" | "prepend"): number {
...
// 插入内容到指定行
function insertContentAtLine(lines: string[], targetLine: number, content: string): string[] {
...
// 处理插入操作
function handleInsertEdit(lines: string[], elements: MarkdownElement[], edit: EditOperation): string[] {
  // 验证操作
  const validationErrors = validateEditOperation(edit);
  if (validationErrors.length > 0) {
    throw new InsertError(validationErrors.join("; "), "validation", edit.heading || edit.blockId || "unknown");
  }
  ...
  try {
    if (edit.heading) {
      ...
      if (!headingResult) {
        throw new InsertError(`Heading not found: ${edit.heading}${edit.level ? ` (level ${edit.level})` : ""}`, "heading_search", edit.heading);
      }
      ...
      if (!blockResult) {
        throw new InsertError(`Block ID not found: ${edit.blockId}`, "block_search", edit.blockId);
      }
    ...
    throw new InsertError(error instanceof Error ? error.message : String(error), "insert_operation", edit.heading || edit.blockId || "unknown");
  }
}
...
export class ObsidianMcpServer {
  ...
  private async handleUpdateNote(args: any) {
    ...
    if (filesystemResult) {
      result += fallbackEdits.length === edits.length ? filesystemResult : `Filesystem operations:\n${filesystemResult}`;
    }
    if (!result) {
  ...
  private async handleNotesInsight(args: any) {
    ...
    try {
      ...
      const processedContent = await this.processNotesContent(selectedNotes, maxContextLength, enableSummary);
  ...
  private async processNotesContent(notes: { path: string; score: number; content: string }[], maxContextLength: number, enableSummary: boolean): Promise<string> {
    let totalLength = 0;
  ...
  private async summarizeNoteContent(notePath: string, content: string): Promise<string> {
    ...
    const summaryRequest = {
      ...
      requirements: ["Key concepts and main ideas", "Important details and insights", "Relevant facts and data points", "Core arguments or conclusions", "Actionable information"],
...
Your goal is to help users see beyond apparent limitations and discover innovative pathways that transcend traditional trade-offs.`;
      ...
      for (const file of files) {
        try {
          ...
          if (matchedByName || matchedByContent) {
            ...
            const lineMatch = matchedByContent && typeof content === "string" ? content.split("\n").findIndex((line) => line.toLowerCase().includes(lowerQuery)) : -1;
  ...
  private async patchNoteViaAPI(notePath: string, heading: string, content: string, position: string): Promise<void> {
    try {
      // 尝试使用 Obsidian Local REST API 的 PATCH 端点
      await this.api.patch(`/vault/${encodeURIComponent(notePath)}`, {
        heading: heading,
        content: content,
        insertionMode: position === "append" ? "append" : position === "prepend" ? "prepend" : position === "before" ? "before" : "after",
      });
    } catch (error) {
  ...
  // 基于块ID的 PATCH 操作
  private async patchNoteViaBlockAPI(notePath: string, blockId: string, content: string, position: string): Promise<void> {
    try {
